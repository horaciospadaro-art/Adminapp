...
const REPORT_TYPES = [
    ...
    { id: 'receivables', label: 'Cuentas por cobrar', icon: Calendar, needsDateRange: false }
] as const
...
export function ClientReportsModal({ companyId, isOpen, onClose }: ClientReportsModalProps) {
    const router = useRouter()
    const [clients, setClients] = useState<{ id: string; name: string; rif: string }[]>([])
    ...
    useEffect(() => {
        ...
        fetch(`/api/operations/clients?companyId=${companyId}`)
            ...
            .then(data => {
                ...
                setSelectedClientId(Array.isArray(data) && data.length > 0 ? data[0].id : '')
            })
            .catch(() => setClients([]))
    ...
    const handleGenerate = () => {
        ...
        const needsRange = selectedReportType === 'statement'
        if (needsRange && (!dateFrom || !dateTo)) return
        onClose()
        const base = `/dashboard/operations/clients/${selectedClientId}`
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
            ...
            >
                <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                    <h2 className="text-lg font-bold text-gray-800">Reportes por cliente</h2>
                    <button
                        type="button"
                        onClick={onClose}
                    ...
                    <>
                        ...
                        <div className="flex gap-2 pt-2">
                            ...
                            <button
                                ...
                                disabled={!selectedClientId || (showDateRange && (!dateFrom || !dateTo))}

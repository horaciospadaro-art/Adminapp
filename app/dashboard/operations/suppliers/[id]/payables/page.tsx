...
const DOC_TYPES_PAYABLES = [DocumentType.BILL, DocumentType.CREDIT_NOTE, DocumentType.DEBIT_NOTE]

export default async function SupplierPayablesPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={`Cuentas por pagar - ${supplier?.name || 'Proveedor'}`}
                backHref="/dashboard/operations/suppliers"
            />
            <p className="text-sm text-gray-600">
                Documentos con saldo pendiente (facturas, notas de crédito y notas de débito).
            </p>
                        ...
                        {documents.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                                    No hay documentos con saldo pendiente para este proveedor.
                                </td>
                            </tr>
                        ) : (
                            documents.map((doc) => {
                                ...
                                return (
                                    <tr key={doc.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right">
                                            <Link
                                                href={`/dashboard/operations/bills/new?id=${doc.id}`}
